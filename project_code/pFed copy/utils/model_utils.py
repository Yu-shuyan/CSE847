...
def read_data(dataset):
    ...
    return clients, groups, train_data, test_data
def read_data_mci(X, y ,groups,test, train):
    clients, client_test, train_data, test_data = read_mci(X, y, groups, test, train)
    return clients, client_test, train_data, test_data

def read_mci(X, y ,groups,test, train):
    # Create data structure
    train_data = {'users': [], 'user_data': {}, 'num_samples': []}
    test_data = {'users': [], 'user_data': {}, 'num_samples': []}
    train_id = {}
    test_id = {}
    user_train = groups[train]
    user_test = groups[test]
    idxs_train = np.unique(user_train)
    idxs_test = np.unique(user_test)
    k = 0
    for u in idxs_train:
        mask = user_train == u
        train_data["user_data"][k] = {'x': X[train[mask], :, :], 'y': y[train[mask]]}
        train_data['users'].append(k)
        k += 1
    k = 0
    for u in idxs_test:
        mask = user_test == u
        test_data["user_data"][k] = {'x': X[test[mask], :, :], 'y': y[test[mask]]}
        test_data['users'].append(k)
        k += 1
    return train_data['users'], test_data['users'], train_data['user_data'], test_data['user_data']
